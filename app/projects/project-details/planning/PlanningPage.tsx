...
import { ChevronRight, Download, TriangleAlert, Pencil, Users, Upload, FileText, Plus, Trash2, X } from "lucide-react";
...
const INITIAL_TASK_SECTIONS: TaskSection[] = [
  {
    ...
    defaultOpen: true,
    rows: [
      { id: "terrassement", name: "Terrassement et fouilles",     unit: "m³",     quantity: 450, team: "Équipe Alpha", teamVariant: "blue",   start: "2024-05-12", end: "2024-05-18", duration: "6j"  },
      { id: "fondations",   name: "Fondations spéciales",         unit: "ml",     quantity: 120, team: "Équipe Gamma", teamVariant: "violet", start: "2024-05-19", end: "2024-05-25", duration: "6j"  },
      { id: "dalle",        name: "Dalle de compression",         unit: "m²",     quantity: 210, team: "Équipe Alpha", teamVariant: "blue",   start: "2024-05-26", end: "2024-05-28", duration: "2j"  },
    ],
  },
  {
    id: "second-oeuvre",
    ...
    defaultOpen: false,
    rows: [
      { id: "menuiseries",    name: "Pose menuiseries extérieures",     unit: "u",  quantity: 14,  team: "Équipe Delta", teamVariant: "amber",  start: "2024-06-10", end: "2024-06-14", duration: "4j"  },
      { id: "cloisonnement",  name: "Cloisonnement plaques de plâtre",  unit: "m²", quantity: 580, team: "Équipe Bêta",  teamVariant: "violet", start: "2024-06-15", end: "2024-06-28", duration: "13j" },
    ],
  },
];
...
const MOCK_REPORT_SECTIONS = [
  { id: "1", title: "Résumé de l'activité", body: "Les travaux de gros œuvre progressent conformément au planning. Le terrassement est achevé à 100% sur la zone nord." },
  { id: "2", title: "Ressources engagées", body: "Équipe Alpha : 5 ouvriers, 1 chef de chantier.\nMatériel : 2 pelleteuses, 1 camion benne." },
  { id: "3", title: "Points de vigilance", body: "Prévoir une intervention supplémentaire pour le ferraillage des fondations suite au léger retard de livraison." },
];

export default function PlanningPage() {
  const [sections, setSections] = useState<TaskSection[]>(INITIAL_TASK_SECTIONS);
  const [openSectionIds, setOpenSectionIds] = useState<Set<string>>(
    () => new Set(INITIAL_TASK_SECTIONS.filter((s) => s.defaultOpen).map((s) => s.id)),
  );
  ...
  const updateTask = (sectionId: string, taskId: string, field: keyof TaskRow, value: any) => {
  ...
  const handleExportPDF = () => {
    ...
    doc.text("RAPPORT DE PLANNING - VILLA HORIZON", 105, 20, { align: "center" });
  ...
  const totalQuantity = useMemo(() => sections.reduce(
    (sum, s) => sum + s.rows.reduce((a, r) => a + Number(r.quantity || 0), 0),
    0,
  ), [sections]);
              ...
              <Dialog.Portal>
                ...
                <Dialog.Content className="report-modal-content">
                  <div className="report-modal-header">
                    <Dialog.Title className="report-modal-title">Aperçu du Rapport</Dialog.Title>
            ...
            <table className="planning-table">
              ...
              {sections.map((section) => {
                ...
                return (
                  <tbody key={section.id}>
                    <tr className="planning-section-row" onClick={() => toggleSection(section.id)}>
                      <td colSpan={8}>
                        <div className="planning-section-header">
                          <div className="planning-section-left">
                            ...
                            <span className="planning-task-count">{section.taskCount}</span>
                          </div>
                          <div className="flex items-center gap-4">
                            <span className={`planning-status-badge planning-status-badge--${section.statusVariant}`}>
                              {section.statusLabel}
                            </span>
                            <button 
                              className="add-task-btn" 
                              onClick={(e) => { e.stopPropagation(); addTask(section.id); }}
                        ...
                        <tr key={row.id} className="planning-task-row">
                          <td className="planning-td">
                            <input 
                              ...
                              value={row.name} 
                              onChange={(e) => updateTask(section.id, row.id, "name", e.target.value)}
                            />
                          </td>
                          <td className="planning-td planning-td--center">
                            <input 
                              ...
                              value={row.unit} 
                              onChange={(e) => updateTask(section.id, row.id, "unit", e.target.value)}
                            />
                          </td>
                          <td className="planning-td planning-td--center">
                            <input
                              ...
                              type="number"
                              value={row.quantity}
                              onChange={(e) => updateTask(section.id, row.id, "quantity", e.target.value)}
                            />
                          </td>
                          <td className="planning-td planning-td--center">
                             <select 
                                ...
                                value={row.team}
                                onChange={(e) => updateTask(section.id, row.id, "team", e.target.value)}
                             >
                          ...
                          <td className="planning-td planning-td--center">
                            <input 
                              ...
                              className="editable-cell-input" 
                              value={row.start} 
                              onChange={(e) => updateTask(section.id, row.id, "start", e.target.value)}
                            />
                          </td>
                          <td className="planning-td planning-td--center">
                            <input 
                              ...
                              className="editable-cell-input" 
                              value={row.end} 
                              onChange={(e) => updateTask(section.id, row.id, "end", e.target.value)}
                            />
                          </td>
                          <td className="planning-td planning-td--center">
                            <input 
                              className="editable-cell-input text-center" 
                              value={row.duration} 
                              onChange={(e) => updateTask(section.id, row.id, "duration", e.target.value)}
                            />
                          </td>
                          <td className="planning-td planning-td--center">
                            <button 
              ...
              <tfoot>
                <tr className="planning-tfoot-row">
                  <td className="planning-tfoot-td planning-tfoot-td--label">TOTAL PROJET</td>
                  <td className="planning-tfoot-td planning-tfoot-td--center">—</td>
                  <td className="planning-tfoot-td planning-tfoot-td--center planning-tfoot-td--accent">
                    {totalQuantity.toLocaleString("fr-FR")} u.
                  </td>
                  <td className="planning-tfoot-td planning-tfoot-td--center" colSpan={5}>

...
export default function DailyPlanning() {
  ...
  const handleTaskSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTaskData({ title: "", description: "", assigned_to: "", category: "General" });
      fetchTasks();
    } catch (err: any) {
  ...
  const tasksByStatus = (statusId: number) => tasks.filter(t => t.status_id === statusId);
          ...
          <div className="kanban-column">
            ...
            <div className="column-content">
              ...
              <div className="add-task-row">
                <button type="button" className="add-task-btn" onClick={() => handleOpenModal(1)}>
                  + Add Task
                </button>
              </div>
            </div>
      ...
      {showTaskModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-[500px] max-w-[90vw] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleTaskSubmit} className="p-6 flex flex-col gap-5">
              <div>
              ...
              <div>
                ...
                >
                  ...
                  {(project?.members ?? []).filter(m => m.user_id !== currentUser?.id).map(member => (

  ...
  return (
    <div>
      <div className="projects-empty">
        <h2>Aucun projet</h2>
        <p>Cette page affichera vos projets une fois créés ou partagés avec vous.</p>
...
function ProjectsList({ projectList = [], hasProjects = true, onCreated }: {
  projectList?: Project[];
  hasProjects?: boolean;
  onCreated: () => void;
}) {
...
function Projects() {
  ...
  const filteredProjects = useMemo(() => {
    ...
    if (activePage === 1) result = result.filter(p => ["active", "on_track", "risk"].includes(p.status));
    if (activePage === 2) result = result.filter(p => p.status === "completed");
    const q = searchQuery.trim().toLowerCase();
    if (q) result = result.filter(p => [p.name, p.description, p.status].join(" ").toLowerCase().includes(q));
    return result;
